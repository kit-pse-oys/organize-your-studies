...
public class PlanningService {
    ...
    /**
     * Wendet eine Strafkosten auf die Kostenmatrix einer Aufgabe an, um eine
     * Neuplanung einer bestimmten Lerneinheit zu fördern.
     *
     * @param task      Die Aufgabe, deren Kostenmatrix angepasst werden soll.
     * @param unit      Die Lerneinheit, die neu terminiert werden soll.
     * @param weekStart Das Startdatum der Woche.
     */
    ...
    private void saveLearningResults(List<PlanningResponseDTO> results, LocalDate weekStart, int breakDuration) {
        ...
        for (PlanningResponseDTO result : results) {
            ...
            if (task != null) {
                ...
                if (!units.isEmpty()) {
                    LearningUnit savedUnit = units.get(units.size() - 1);
