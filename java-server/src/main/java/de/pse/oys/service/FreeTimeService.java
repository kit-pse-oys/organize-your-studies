...
import de.pse.oys.dto.FreeTimeDTO2;
...
public class FreeTimeService {
     ...
     * @param dto Eingabedaten (temporär: {@link FreeTimeDTO2})
     * @return angelegte Freizeit als DTO
     */
    public FreeTimeDTO2 createFreeTime(UUID userId, FreeTimeDTO2 dto) {
     ...
     */
    public FreeTimeDTO2 updateFreeTime(UUID userId, FreeTimeDTO2 dto) {
        User user = loadUser(userId);
    ...
    private void validateData(User user, FreeTimeDTO2 dto, UUID ignoreId) {
        if (dto == null
    ...
    private FreeTime mapToEntity(UUID id, FreeTimeDTO2 dto) {
        if (dto.isWeekly()) {
    ...
    private FreeTimeDTO2 mapToDto(FreeTime ft) {
        ...
        return new FreeTimeDTO2(ft.getFreeTimeId(), ft.getTitle(), date, ft.getStartTime(), ft.getEndTime(), weekly);
    }
